...
const planRank:Record<string,number>={listed:1,noticed:2,featured:3,everywhere:4};
...
export async function POST(request: Request) {
  ...
  try {
    const object=event.data?.object||{};const businessId=object.metadata?.business_id||object.subscription_details?.metadata?.business_id||null;const planCode=object.metadata?.plan_code||object.subscription_details?.metadata?.plan_code||null;
    if(event.type==="checkout.session.completed"){
      ...
      if(businessId)await admin.from("businesses").update({review_status:"submitted",submitted_at:new Date().toISOString(),current_plan_code:planCode||null,promotion_rank:planCode?planRank[planCode]||0:0,updated_at:new Date().toISOString()}).eq("id",businessId);
    }
    if(event.type==="invoice.paid"||event.type==="invoice.payment_failed"){
      ...
      if(subscriptionId)await admin.from("business_plan_subscriptions").update({status:event.type==="invoice.paid"?"active":"past_due",payment_status:event.type==="invoice.paid"?"paid":"failed",updated_at:new Date().toISOString()}).eq("stripe_subscription_id",subscriptionId);
    }
    if(event.type==="customer.subscription.updated"||event.type==="customer.subscription.deleted"){
      ...
      const {data:sub}=await admin.from("business_plan_subscriptions").update({status:nextStatus,cancel_at_period_end:Boolean(s.cancel_at_period_end),current_period_end:s.current_period_end?new Date(s.current_period_end*1000).toISOString():null,updated_at:new Date().toISOString()}).eq("stripe_subscription_id",s.id).select("business_id,plan_code").maybeSingle();
      if(sub){const active=nextStatus==="active";await admin.from("businesses").update({current_plan_code:active?sub.plan_code:null,promotion_rank:active?(planRank[sub.plan_code]||0):0,updated_at:new Date().toISOString()}).eq("id",sub.business_id);}
    }
