...
type Ad={id:string;headline:string;format:string;impression_count:number;video_play_count:number;click_count:number;is_active:boolean};
type Deal={id:string;title:string;view_count:number;claim_count:number;is_active:boolean};

export default function AdvertiserDashboard(){
 const [business,setBusiness]=useState<Business|null>(null);const [ads,setAds]=useState<Ad[]>([]);const [deals,setDeals]=useState<Deal[]>([]);const [loading,setLoading]=useState(true);
 useEffect(()=>{(async()=>{const s=createClient();const {data:u}=await s.auth.getUser();if(!u.user){window.location.href="/advertise/signup";return;}const {data:b}=await s.from("businesses").select("id,name,category,profile_view_count,is_verified").eq("owner_user_id",u.user.id).order("created_at",{ascending:false}).limit(1).maybeSingle();if(!b){window.location.href="/advertise/setup";return;}setBusiness(b);const [{data:a},{data:d}]=await Promise.all([s.from("business_ads").select("id,headline,format,impression_count,video_play_count,click_count,is_active").eq("business_id",b.id).order("created_at",{ascending:false}),s.from("deals").select("id,title,view_count,claim_count,is_active").eq("business_id",b.id).order("created_at",{ascending:false})]);setAds(a??[]);setDeals(d??[]);setLoading(false);})();},[]);
