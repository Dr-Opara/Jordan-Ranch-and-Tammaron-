...
export default function BusinessSetupPage() {
  ...
  async function submit(e:FormEvent){
    e.preventDefault();setError("");if(!terms||!privacy){setError("You must accept the Business Terms and Privacy Policy before submitting.");return;}setLoading(true);
  ...
  return <main className="auth-page"><section className="auth-card wide">
    <Link href="/advertise" className="auth-brand">JRT.Community</Link><span className="badge sponsored">Free Business Listing</span><h1>Tell residents about your business</h1>
    <p className="auth-copy">Complete the information residents need to evaluate your business. There is no listing fee. Your profile will remain private until it passes business review; after approval you may optionally bid for sponsored leaderboard placement.</p>
    <form onSubmit={submit} className="form-stack">
      ...
      <label><span><input type="checkbox" checked={terms} onChange={e=>setTerms(e.target.checked)}/> I certify the business information is accurate and agree to the Business and Sponsored Leaderboard Terms.</span></label>
      ...
      {error&&<div className="form-error">{error}</div>}<button className="btn-primary" disabled={loading}>{loading?"Submitting…":"Submit Free Business Listing"}</button>
    </form>
  </section></main>;
}